...
**ComfyUI bump ritual**: update the `comfyui` manifest item → re-apply the marked `Stillsong:` blocks to the new `ar.py` and update `PINNED_AR_SHA256` in `stillsong_ar.py` (the node refuses to load on mismatch) → `npm run lock:python` → `npm run check:manifest` → `node scripts/overlay-smoke.mjs` → one real render.
...
- KSampler `steps` touches only the diffusion pass (timbre, clarity); the composition is fixed upstream by seed + inputs. Defaults live in `RENDER_STEPS` (`fast` 40 for new songs, `enhanced` 80). "Enhance quality" re-renders at 80 with the saved codes forced and, at harvest, takes the original row's place (`replaces_song_id` → `SongRepo.takePlaceOf`) instead of adding a version — but only when the composition is provably preserved (both SSC1 blobs byte-equal; or no codes and the original was a free render). Otherwise it stays as a new version. The verdict is persisted (`replace_verified`) before anything is deleted, the marker is cleared last, and `finishReplacements` completes an interrupted swap at boot; `enhanceSong` returns the pending re-take instead of queuing a second. It cannot skip the encode stage: the DiT consumes the encoder's live hidden states, which are never stored — and the encode node's inputs differ (codes prefix, save prefix), so ComfyUI's output cache never helps.
