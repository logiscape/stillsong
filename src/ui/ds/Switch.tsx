...
export function Switch({ checked, onChange, label, hint, style }: SwitchProps): React.ReactElement {
  return (
    <label style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-4)', cursor: 'pointer', ...style }}>
      <button
        type="button" role="switch" aria-checked={!!checked} onClick={() => onChange && onChange(!checked)}
        style={{
          position: 'relative', flex: '0 0 auto', width: 40, height: 24, marginTop: 1, borderRadius: 'var(--radius-pill)',
