...
const SIZES = {
  sm: { h: 32 as number | string, px: 12, font: 'var(--ui-sm)', gap: 6, icon: 15 },
  md: { h: 'var(--control-h)' as number | string, px: 18, font: 'var(--ui-button)', gap: 8, icon: 17 },
  lg: { h: 'var(--control-h-lg)' as number | string, px: 26, font: '500 15.5px/1 var(--font-ui)', gap: 9, icon: 18 },
};
...
export function Button({ variant = 'primary', size = 'md', icon, iconRight, disabled, fullWidth, onClick, children, style }: ButtonProps): React.ReactElement {
  ...
  return (
    <button
      ...
      style={{
        display: fullWidth ? 'flex' : 'inline-flex', width: fullWidth ? '100%' : undefined, alignItems: 'center',
